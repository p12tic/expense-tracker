    ...
    setAccounts((prevAccounts) =>
      prevAccounts.map((acc) =>
        acc.id === clickedAccUse.id
          ? {...acc, fraction: (parseFloat(acc.fraction) + step).toFixed(2)}
          : acc,
      ),
    );
        ...
        setAccounts((prevAccounts) =>
          prevAccounts.map((acc) =>
            acc.id === clickedAccUse.id
              ? {
                  ...acc,
                  fraction: (parseFloat(acc.fraction) + step).toFixed(2),
                }
              : acc,
          ),
        );
    ...
    setAccounts((prevAccounts) =>
      prevAccounts.map((acc) =>
        acc.id === accMulti.id
          ? {
              ...acc,
              fraction: (acc.fraction = parseFloat(e.target.value).toString()),
            }
          : acc,
      ),
    );
